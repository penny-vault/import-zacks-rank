...
func (balanceSheetList BalanceSheetList) SaveToParquet(fn string) error {
	var err error
	...
	pw, err := writer.NewParquetWriter(fh, new(BalanceSheetRecord), 4)
	if err != nil {
		log.Error().
			Str("OriginalError", err.Error()).
			Msg("Parquet write failed")
	...
	for _, r := range balanceSheetList {
		if err = pw.Write(r); err != nil {
			log.Error().
				Str("OriginalError", err.Error()).
				Str("CalendarDate", r.CalendarDate).
				Str("Ticker", r.Ticker).
				Str("CompositeFigi", r.CompositeFigi).
				Msg("Parquet write failed for record")
		}
	...
	log.Info().Int("NumRecords", len(balanceSheetList)).Msg("Parquet write finished")
	return nil
}
