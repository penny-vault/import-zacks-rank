...
type ZacksRecord struct {
	...
	EventDate                                 string  `csv:"-" json:"event_date" parquet:"name=event_date, type=BYTE_ARRAY, convertedtype=UTF8, encoding=PLAIN_DICTIONARY" db:"event_date,omitempty"`
	...
	LastReportedQtrDate                       string  `csv:"Last Reported Qtr (yyyymm)" json:"last_reported_qtr_date" parquet:"name=last_reported_qtr_date, type=BYTE_ARRAY, convertedtype=UTF8, encoding=PLAIN_DICTIONARY" db:"last_reported_qtr_date,omitempty"`
	...
	LastReportedFiscalYr                      string  `csv:"Last Reported Fiscal Yr  (yyyymm)" json:"last_reported_fiscal_yr" parquet:"name=last_reported_fiscal_yr, type=BYTE_ARRAY, convertedtype=UTF8, encoding=PLAIN_DICTIONARY" db:"last_reported_fiscal_yr,omitempty"`
	LastEpsReportDate                         string  `csv:"Last EPS Report Date (yyyymmdd)" json:"last_eps_report_date" parquet:"name=last_eps_report_date, type=BYTE_ARRAY, convertedtype=UTF8, encoding=PLAIN_DICTIONARY" db:"last_eps_report_date,omitempty"`
	NextEpsReportDate                         string  `csv:"Next EPS Report Date  (yyyymmdd)" json:"next_eps_report_date" parquet:"name=next_eps_report_date, type=BYTE_ARRAY, convertedtype=UTF8, encoding=PLAIN_DICTIONARY" db:"next_eps_report_date,omitempty"`
...
func LoadRatings(ratingsFn string, dateStr string, limit int) []*ZacksRecord {
	...
	for _, r := range records {
		...
		r.EventDate = dateStr
		// parse date fields
		dt, err := time.Parse("200601", r.LastReportedFiscalYr)
		if err == nil {
			r.LastReportedFiscalYr = dt.Format("2006-01-02")
		} else {
			if r.LastReportedFiscalYr != "" {
				log.Warn().Str("Ticker", r.Ticker).Str("InputString", r.LastReportedFiscalYr).Msg("could not parse last reported fiscal year")
			}
		}

		dt, err = time.Parse("200601", r.LastReportedQtrDate)
		if err == nil {
			r.LastReportedQtrDate = dt.Format("2006-01-02")
		} else {
			if r.LastReportedQtrDate != "" {
				log.Warn().Str("Ticker", r.Ticker).Str("InputString", r.LastReportedQtrDate).Msg("could not parse last reported quarter date")
			}
		}

		dt, err = time.Parse("20060102", r.LastEpsReportDate)
		if err == nil {
			r.LastEpsReportDate = dt.Format("2006-01-02")
		} else {
			if r.LastEpsReportDate != "" {
				log.Warn().Str("Ticker", r.Ticker).Str("InputString", r.LastEpsReportDate).Msg("could not parse last eps report date")
			}
		}

		dt, err = time.Parse("20060102", r.NextEpsReportDate)
		if err == nil {
			r.NextEpsReportDate = dt.Format("2006-01-02")
		} else {
			if r.NextEpsReportDate != "" {
				log.Warn().Str("Ticker", r.Ticker).Str("InputString", r.NextEpsReportDate).Msg("could not parse next eps report date")
			}
		}
...
func EnrichWithFigi(records []*ZacksRecord) []*ZacksRecord {
	settings, err := postgresql.ParseURL(viper.GetString("DATABASE_URL"))
	if err != nil {
		log.Error().Str("OriginalError", err.Error()).Msg("Could not parse database connection string")
	}
	...
	rows, err := session.SQL().Query("SELECT ticker, name, composite_figi FROM tickers_v1 WHERE active='t' AND composite_figi IS NOT NULL")
	if err != nil {
...
func SaveToDB(records []*ZacksRecord) {
	...
	collection := session.Collection("zacks_financials_v1")
	cnt := 0
	for _, r := range records {
		if r.CompositeFigi != "" {
			res := collection.Find(db.Cond{"composite_figi": r.CompositeFigi}).And(db.Cond{"event_date": r.EventDate}).And(db.Cond{"ticker": r.Ticker})
			numMatch, err := res.Count()
			if err != nil {
				log.Warn().Str("OriginalError", err.Error()).Msg("could not check if record exist")
			}
			if numMatch == 0 {
				_, err := collection.Insert(r)
